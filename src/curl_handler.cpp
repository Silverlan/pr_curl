...
void CurlHandler::StartDownload()
{
	...
	m_thread = std::make_unique<std::thread>([this]() {
			...
			while(code == CURLE_OK && handleCount > 0 && m_bCancel == false) {
				...
				while((msg = curl_multi_info_read(m_multiCurl, &msgsLeft)) != nullptr) {
					if(msg->msg == CURLMSG_DONE) {
						...
						auto it = std::find_if(m_activeRequests.begin(), m_activeRequests.end(), [curl](const std::shared_ptr<CurlHandler::Request> &request) { return (request->handle.get() == curl) ? true : false; });
						auto released = false;
		...
		for(auto &req : m_activeRequests) {
			curl_multi_remove_handle(m_multiCurl, req->handle.get());
			req->headers = nullptr;
			req->handle = nullptr;
		}
...
void CurlHandler::AddResource(const std::string &url, const std::function<size_t(void *, size_t, size_t)> &callback, const std::shared_ptr<void> &userData, const std::function<void(int64_t, int64_t, int64_t, int64_t)> &progressCallback, const std::function<void(int32_t)> &onComplete)
{
...
void RequestData::SetPostKeyValues(const std::unordered_map<std::string, std::string> &postValues)
{
	std::string post;
	auto bFirst = true;
	...
	postData = std::move(post);
}

void CurlHandler::AddRequest(const std::string &url, const RequestData &requestData)
{
	auto post = std::move(requestData.postData);
	auto response = std::make_shared<std::vector<uint8_t>>();
	auto &onComplete = requestData.onComplete;
	auto &progressCallback = requestData.progressCallback;
	auto &headers = requestData.headers;
	AddRequest(
	  url,
	  [response](void *ptr, size_t size, size_t nmemb) -> size_t {
		  auto offset = response->size();
		  response->resize(offset + size * nmemb);
		  memcpy(&(*response)[offset], ptr, size * nmemb);
		  return size * nmemb;
	  },
	  [response, onComplete = std::move(onComplete)](int32_t code, const std::string &fileName) { onComplete(code, *response); },
	  [progressCallback = std::move(progressCallback)](int64_t dltotal, int64_t dlnow, int64_t ultotal, int64_t ulnow) {
		  if(progressCallback == nullptr)
			  return;
		  progressCallback(dltotal, dlnow, ultotal, ulnow);
	  },
	  [post = std::move(post), headers = std::move(headers)](Request *pReq, void *multiCurl) {
		  auto *curl = curl_easy_init();
		  if(curl != nullptr) {
			  InitializeCurl(curl, pReq);
			  ...
			  if(!headers.empty()) {
				  ...
				  for(auto &header : headers) {
					  cheaders = curl_slist_append(cheaders, header.c_str());
				  }
				  curl_easy_setopt(curl, CURLOPT_HTTPHEADER, cheaders);
				  pReq->headers = std::shared_ptr<curl_slist> {cheaders, [](curl_slist *cheaders) { curl_slist_free_all(cheaders); }};
			  }
